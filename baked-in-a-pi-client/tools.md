# The Tools Used in `baked-in-a-pi`

## Introduction

`baked-in-a-pi` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `baked-in-a-pi` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`baked-in-a-pi` uses git to manage its codebase. While in theory you don't have
to use Git once you download `baked-in-a-pi`, this project makes the assumption
...
While `baked-in-a-pi` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `baked-in-a-pi` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `baked-in-a-pi`; Grunt is
...
to get started with `baked-in-a-pi`. But for those curious or looking to go a
...
Grunt is the engine behind `baked-in-a-pi`. It's the magic that makes it move.
...
`baked-in-a-pi` comes with a `bower.json` file that looks something like this:
...
in `baked-in-a-pi` by default in there to see what I mean.
